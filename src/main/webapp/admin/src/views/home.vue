...
<div class="content" :style='{"minHeight":"100vh","padding":"0px 20px 20px","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230830/d58ecc944fb54dbfa18ecd2b64e92170.png) no-repeat center top / cover","display":"flex","fontSize":"16px"}'>
	...
	<div :style='{"margin":"0 20px 30px 0","alignItems":"center","color":"#fff","flexWrap":"wrap","display":"flex","width":"240px","fontSize":"inherit","justifyContent":"center"}'>
		...
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","margin":"0 10px 16px 0","borderRadius":"4px","background":"rgba(86,178,198,.6)","display":"flex","clipPath":"polygon(95% 0, 100% 50%, 95% 100%, 0% 100%, 0 50%, 0% 0)","width":"100%"}' v-if="isAuth('yingpinxinxi','首页总数')">
			<div :style='{"width":"60px","alignItems":"center","background":"none","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-tongji5" :style='{"color":"inherit","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"calc(100% - 60px)","alignItems":"center","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{yingpinxinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>应聘信息总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 3 -->
	<div class="type3" :style='{"alignContent":"flex-start","padding":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="yonghuChart1" class="echarts1" v-if="isAuth('yonghu','首页统计')"></div>
		<div id="yingpinxinxiChart1" class="echarts2" v-if="isAuth('yingpinxinxi','首页统计')"></div>
		<div id="yingpinxinxiChart2" class="echarts3" v-if="isAuth('yingpinxinxi','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	methods:{
        ...
        var yonghuChart1 = echarts.init(document.getElementById("yonghuChart1"),'dark');
        this.$http({
            url: "yonghu/group/xueli",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
        ...
        var yingpinxinxiChart1 = echarts.init(document.getElementById("yingpinxinxiChart1"),'caravan');
        this.$http({
            url: "yingpinxinxi/group/gangweimingcheng",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
    ...
    yingpinxinxiChat2() {
      this.$nextTick(()=>{
        ...
        var yingpinxinxiChart2 = echarts.init(document.getElementById("yingpinxinxiChart2"),'caravan');
        this.$http({
            url: "yingpinxinxi/group/gangweifenlei",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    xAxis.push(res[i].gangweifenlei);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].gangweifenlei
                    })
                }
                var option = {};
                let titleObj = this.pie.title
				titleObj.text = '应聘岗位分类'
                ...
                // 使用刚指定的配置项和数据显示图表。
                yingpinxinxiChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yingpinxinxiChart2.resize();
                };
            }
        });
      })
