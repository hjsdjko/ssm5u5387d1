		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 0px","fontSize":"inherit","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"0 50px 10px 0","fontSize":"inherit","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="企业性质" prop="qiyexingzhi">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">企业性质</label>
						<el-select clearable v-model="searchForm.qiyexingzhi" placeholder="请选择企业性质" >
							<el-option v-for="(item,index) in qiyexingzhiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
			...
			<div :style='{"width":"100%","padding":"0","background":"none"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='true' prop="qiyelogo" width="200" label="企业logo">
						<template slot-scope="scope">
							<div v-if="scope.row.qiyelogo">
								<img v-if="scope.row.qiyelogo.substring(0,4)=='http'" :src="scope.row.qiyelogo.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.qiyelogo.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true' prop="content" label="状态">
						<template slot-scope="scope">
                            <el-switch
								...
								@change="(e)=>qiyestatusChange(e,scope.row)">
                            </el-switch>
						</template>
					</el-table-column>
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.qiyexingzhi!='' && this.searchForm.qiyexingzhi!=undefined){
            params['qiyexingzhi'] = this.searchForm.qiyexingzhi
          }
