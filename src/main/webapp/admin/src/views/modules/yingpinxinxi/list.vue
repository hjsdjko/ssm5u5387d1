<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 0px","fontSize":"inherit","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"0 0 10px","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('yingpinxinxi','岗位应聘次数')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"#fff","height":"40px"}'></span>
						岗位应聘次数
					</el-button>
					<el-button class="btn18" v-if="isAuth('yingpinxinxi','应聘岗位分类')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"#fff","height":"40px"}'></span>
						应聘岗位分类
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"0","background":"none"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- 无 -->
					<el-table-column :resizable='true' :sortable='true' prop="gongzuohuanjing" width="200" label="工作环境">
						<template slot-scope="scope">
							<div v-if="scope.row.gongzuohuanjing">
								<img v-if="scope.row.gongzuohuanjing.substring(0,4)=='http'" :src="scope.row.gongzuohuanjing.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.gongzuohuanjing.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yingpinxinxi','面试通知')" @click="mianshitongzhiCrossAddOrUpdateHandler(scope.row,'cross','是','','[1]','已通知')" type="success">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="gangweifenleiChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var gangweimingchengChart1 = echarts.init(document.getElementById("gangweimingchengChart1"),'caravan');
        this.$http({
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var gangweifenleiChart2 = echarts.init(document.getElementById("gangweifenleiChart2"),'caravan');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                gangweifenleiChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    gangweifenleiChart2.resize();
                };
            }
        });
      })
